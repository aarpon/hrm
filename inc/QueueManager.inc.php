...
class QueueManager {
  ...
  */
  ...
  /*!
   ...
   \brief       to avoid conflicts between the previews generated from
   \brief       the website and the ones generated from the queue manager, as
  ...
  */
   ...
   */
 	...
 	/*!
 	 ...
 	 \todo	This method in linux works only if the QueueManager is run by root! On Mac OS X this should work!
 	 */
 	...
 	/*!
 	 \brief	Writes a summary of all parameters used to file and appends the
 	 content of Job log and error files.

 	 This file will then be sent to the User by email.
 	 ...
 	 \todo	In the future, more information should be added. The file should
 	 also be visible from the File Browser.
 	 */
 	public function writeParameterFile($job, $startTime, $logFile, $errorFile) {
 	  global $resultImagesOwnedByUser;
 	  global $imageProcessingIsOnQueueManager;
 	  $result = False;
 	  $desc = $job->description();
 	...
 	public function notifySuccess($job, $startTime) {
      ...
      // Remarks
	  $destFolder = $fileserver->destinationFolder();
 	  $remarksFile = $destFolder. "/". $imageName.".remarks.txt";
 	...
 	public function notifyError($job, $message, $startTime) {
	  ...
	  $mail->setSubject( 'An HRM job from user "' . $user->name() . '" finished with an error.' );
 	  $mail->send();
 	}

 	/*!
 	 \brief	Sends an e-mail to the User notifying interactive dialogs silently managed by Huygens
 	 \param	$job		A Job object
 	 \param	$message	An optional, additional warning message.
 	 \param	$startTime	Start time of the Job
 	 \todo	This is currently not used!
 	 */
 	public function notifyWarning($job, $message, $startTime) {
 	  global $email_sender;
 	  $job->createScript();
 	  $desc = $job->description();
 	  $user = $desc->owner();
	  ...
	  $text = $text . "These are the parameters you set in the HRM:\n\n";
 	  $mail = new Mail($email_sender);
 	  $mail->setReceiver($emailAddress);
 	  $mail->setSubject('Your HRM job finished with a warning');
 	  $mail->setMessage($text);
 	  $mail->send();
 	}
 	...
 	/*!
 	 \brief	Reports (and internally store) the name of a free server that can accept a Job
