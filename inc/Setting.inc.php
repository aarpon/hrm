...
class ParameterSetting extends Setting {
  ...
  public function displayString() {
    ...
    // Not everything needs to be displayed, either because the Parameter might
    // be only internally used, or because it does not make sense for the current
    // Setting (e.g. it does not make sense to display the pinhole size if the
    // microscope type is 'widefield'.
    foreach ($this->parameter as $parameter) {
      ...
      if ( !$this->isThreeDimensional() && $parameter->name() == 'ZStepSize' )
        continue;
      if ( !$this->isTimeSeries() && $parameter->name() == 'TimeInterval' )
        continue;
      if ( !$this->isNipkowDisk() && $parameter->name() == 'PinholeSpacing' )
        continue;
      ...
      if ( $parameter->name() == 'Binning' ) // This is obsolete
        continue;
      if ( $parameter->name() == "AberrationCorrectionNecessary" && $PSFmode == 'measured' )
        continue;
      if ( $parameter->name() == 'CoverslipRelativePosition' && $PSFmode == 'measured' )
        continue;
...
class TaskSetting extends Setting {
  ...
  /*!
   ...
   \todo	Refactor!
   */
  private function displayStringWithoutOutputFileFormat( $numberOfChannels = 0 ) {
...
/*!
 ...
 A JobParameterSetting is a parameter setting that
 is used when a job is executed by the queue manager.
 It uses different database tables and knows how to put
